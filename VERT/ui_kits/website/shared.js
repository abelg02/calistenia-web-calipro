...
(function () {
  ...
  if (article) {
    ...
    window.addEventListener('scroll', onRead, { passive: true }); onRead();
  }

  // ---------- Wishlist (localStorage) ----------
  const WL_KEY = 'vx_wishlist';
  const getWishlist = () => { try { return JSON.parse(localStorage.getItem(WL_KEY)) || []; } catch(e) { return []; } };
  ...
  const PAGES = [
    { t: 'Inicio', u: 'index.html', c: 'Página' },
    { t: 'Skills', u: 'skills.html', c: 'Página' },
    { t: 'One Arm Handstand', u: 'skills/one-arm-handstand.html', c: 'Skill' },
    { t: 'Planche', u: 'skills/planche.html', c: 'Skill' },
    { t: 'Front Lever', u: 'skills/front-lever.html', c: 'Skill' },
    { t: 'Human Flag', u: 'skills/human-flag.html', c: 'Skill' },
    { t: 'Maltese', u: 'skills/maltese.html', c: 'Skill' },
    { t: 'Método', u: 'metodo.html', c: 'Página' },
    { t: 'Diario', u: 'diario.html', c: 'Página' },
    { t: 'El pino a una mano no empieza por el hombro', u: 'diario/pino-una-mano.html', c: 'Artículo' },
    { t: 'Comer para sostener, no para inflar', u: 'diario/comer-para-sostener.html', c: 'Artículo' },
    { t: 'Front Lever: el error invisible de los codos', u: 'diario/front-lever-codos.html', c: 'Artículo' },
    { t: 'Tienda', u: 'tienda.html', c: 'Página' },
    { t: 'Hoodie Oversized · Carbon', u: 'tienda/hoodie-carbon.html', c: 'Producto' },
    { t: 'Grips de calistenia · Tan', u: 'tienda/grips-tan.html', c: 'Producto' },
    { t: 'App móvil', u: 'app.html', c: 'Página' },
    { t: 'Test de nivel', u: 'test-de-nivel.html', c: 'Página' },
    { t: 'Contacto', u: 'contacto.html', c: 'Página' },
  ];
  ...
  const cpOpen = () => { cp.classList.add('open'); cpInput.value=''; cpRender(''); cpInput.focus(); };
  const cpClose = () => cp.classList.remove('open');
  document.addEventListener('keydown', (e) => {
    if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'k') { e.preventDefault(); cp.classList.contains('open') ? cpClose() : cpOpen(); }
    if (e.key === 'Escape') cpClose();
    if (cp.classList.contains('open')) {
      const links = [...cpList.querySelectorAll('.cmdk-item')];
      if (e.key === 'ArrowDown') { e.preventDefault(); cpSel = Math.min(cpSel+1, links.length-1); links.forEach((l,i)=>l.classList.toggle('sel', i===cpSel)); links[cpSel]?.scrollIntoView({block:'nearest'}); }
      if (e.key === 'ArrowUp') { e.preventDefault(); cpSel = Math.max(cpSel-1, 0); links.forEach((l,i)=>l.classList.toggle('sel', i===cpSel)); links[cpSel]?.scrollIntoView({block:'nearest'}); }
      if (e.key === 'Enter') { links[cpSel]?.click(); }
    }
  });
