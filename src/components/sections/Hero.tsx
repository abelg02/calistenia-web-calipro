...
export function Hero({ hero, cta }: { hero: Dictionary["hero"]; cta: Dictionary["cta"] }) {
  return (
    ...
    >
      <div>
        <p style={{ "--i": 0 } as React.CSSProperties} className="rise mb-5 text-xs font-semibold uppercase tracking-[0.22em] text-gold">{hero.brand}</p>
        <h1 style={{ "--i": 1 } as React.CSSProperties} className="rise font-display text-balance text-[clamp(3rem,10.5vw,6.25rem)] text-travertine">
          <span className="block">{hero.titleA}</span>
          <span className="block">
            {hero.titleB} <em className="inline-block pb-1 italic text-gold">{hero.titleEm}</em>
          </span>
        </h1>
        <p style={{ "--i": 2 } as React.CSSProperties} className="rise mt-6 max-w-[46ch] text-base leading-relaxed text-travertine/85 sm:text-lg">{hero.sub}</p>
        <div style={{ "--i": 3 } as React.CSSProperties} className="rise mt-9 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
